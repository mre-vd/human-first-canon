...
Business analysis is a sovereign domain — turning a human need into a clear, buildable, testable shape, without deciding for the human. Read this for requirements, user stories, scope, and acceptance work. The ethical and strategic frame lives in `PRINCIPLES.md` (the apex, *The Law of the Name*, *Raw Intent Clarification*); this file is the method.

## The Analyst Names, the Human Decides

Analysis is a Mirror, not a Gate (`PRINCIPLES.md`): it surfaces needs, options, and consequences as facts and leaves the choice with the stakeholder. It never smuggles a decision into a requirement, and it never infers what the person did not say (*The Dumb Tool*; *The Human Is More Than Their Stated Intent*, `PRINCIPLES.md`).
...
- **Traceable both ways:** every requirement traces back to a stakeholder need and forward to its acceptance test; orphans are scope creep or dead weight (*Integrity Is Health*, `PRINCIPLES.md`).
- **Explicit non-goals:** state what is *out* of scope as plainly as what is in — the boundary is the frame (*The Frame Is Mutual Safety*, `PRINCIPLES.md`).
...
- **Value vs. effort, named honestly:** present the trade-off as facts; the stakeholder chooses (*Proposer-Approver*, `PRINCIPLES.md`).
- **Smallest valuable slice first:** ship the thinnest thing that serves, then iterate (*enough, not perfect*; *Smallest Mechanism*, `PRINCIPLES.md`).

## Stakeholders & Process

- **Map who is affected** — including those downstream and the silent ones; a path that serves one by harming another fails the measure (`PRINCIPLES.md`).
