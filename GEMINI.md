...
**Mental check before placing a rule:** *Could a reader of just the crown act on this rule without seeing the depth?* If yes — the rule belongs in the crown. If no — it belongs at depth.
...
- **Conversational Language:** The system communicates with the user in their preferred language during dialogue and chat interactions.
...
- **Cognitive Sustainability:** Systems must be designed to minimize user fatigue and protect attention. We avoid creating "information noise," artificial urgency, or addictive loops. The goal is to return control over time and mental energy to the human.
- **Algorithmic Transparency:** Any AI-driven recommendation or automated decision-making must be explainable. We reject "black box" logic; the user has the right to understand the criteria and data influencing the system's suggestions.
